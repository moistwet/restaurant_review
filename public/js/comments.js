    ...
    for (var i = 0; i < comment_array.length; i++) {
        if (comment_array[i].restaurantid=== restaurant_array[item].restaurantid) {
            ...
            var html = '<div class="text-center" style="width:100%;">                                                           \
                            <div class="card">                                                                                  \
                                <div class="card-body">                                                                         \
                                    <p class="card-text" id="rating' + i + '">' + comment_array[i].reviewtxt + "</p>               \
                                    <small>by " + comment_array[i].username + " @ " + comment_array[i].date_posted + "</small>   \
                                </div>                                                                                          \
                            </div>                                                                                              \
                        </div>";
            ...
            for (var j = 0; j < comment_array[i].rating; j++) {
                console.log(i);
                star += "<img src='images/isaac.jpg' style='width:50px' />";
            }
...
function addComment() {
    ...
    comment.logo = "images/something"
    comment.reviewtxt = document.getElementById("userComments").value; // Value from HTML input text
    comment.rating = rating;
    comment.profileid = 1;
    comment.restaurantid = restaurant_array[currentIndex].restaurantid; // restaurant ID is required by server to create new comment 
    //comment.restaurant = restaurant_array[currentIndex].name; // restaurant title is required by server to create new comment
    comment.date_Posted = ""; // Change the datePosted to null instead of taking the timestamp on the client side;
    console.log(comment)
...
function editComment(element) {
    ...
    document.getElementById("edituserComments").value = comment_array[item].reviewtxt;
...
function updateComment() {
    ...
    if (response == true) {
        ...
        comment_array[currentIndex].reviewtxt = document.getElementById("edituserComments").value;
